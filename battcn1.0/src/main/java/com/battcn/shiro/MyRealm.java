...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		if (userName != null) {
			...
			List<ResourcesEntity> rs = new ArrayList<ResourcesEntity>();//资源权限集合
			// 权限信息对象info,用来存放查出的用户的所有的角色（role）及权限（permission）
			SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
			List<RoleEntity> roles = roleService.findRoleByUser(userId);//获取角色
			...
			for (RoleEntity role : roles) {
				...
				if (StringUtils.isNotBlank(roleId)) {
					hs.add(roleId);
					// 角色对应相应的权限
					rs = resourcesService.findResourcesByRoleId(roleId);//根据角色获取权限集合
					for (ResourcesEntity resources : rs) {
						info.addStringPermission(resources.getResUrl());//注入权限 这里是url
					}
				}
			}
			info.setRoles(hs);//输入角色
			return info;
		}
		return null;
	}
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) {
		...
		if (user != null) {
			...
			SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(accountName, user.getPassWord().toCharArray(), ByteSource.Util.bytes(accountName + "" + user.getCredentialsSalt()), getName());
			// bWVtbXNj

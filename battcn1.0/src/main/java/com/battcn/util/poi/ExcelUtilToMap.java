...
public class ExcelUtilToMap {
	 ...
	 * @throws Exception
	 */
	public static List<Map<String, String>> readExcelFileMap(String sRealPath, int iSheetNum, String key[], boolean isTitle) throws Exception {
		...
		for (int i = 0; i < iSheetNum; i++, iRowNum = 0) {
			...
			/**
			 * 获取当前SHEET的行数（即使SHEET是空的，它默认也是记录数0+1=1） 这个说明了getLastRowNum()只获取最后一个行数，不管前面是否有空行
			 */
				...
				for (int k = 0; k < iCellNum; k++) {
					...
					case Cell.CELL_TYPE_NUMERIC: { // 文本类型
						...
						if (DateUtil.isCellDateFormatted(cellTmp)) {
							...
							/**
							 * 由于API版本问题，不能识别"2007年07月14日","2007-06-02"这种格式的日期数据 所以需要判断它的style,以进一步识别该日期格式 (有待改善)
							 */
							...
							if (idd == 185 || idd == 189 || idd == 188) {
								...
								aValues.put(key[k], String.valueOf(NumberFormat.getInstance().format(d).replace(",", "")));
							}
						}
						break;
					}
	 ...
	 * @throws Exception
	 */
	public static List<Map<String, String>> readExcelFileMap(InputStream is, int iSheetNum, String key[], boolean isTitle) throws Exception {
	 ...
	 *            //EXCEL 数据集合
	 * @param sTypes
	 *            //写入EXCEL文件的数据样式(文本,数字,日期等等)例如,"String","Numeric","yyyy-MM-dd" ；若为NULL，则默认为文本
	 * @param isTitle
	 ...
	 * @author
	 */
	public static void createExcelFile(Workbook workBook, String sFileName, int iTotalSheetNum, String sNameTmp, List<Map<String, String>> aList, String header[], String sTypes[], HttpServletResponse response) throws Exception {
		...
		if (aList != null && aList.size() > 0) {
			...
			for (int i = 0; i < len; i++, iCountNum++) {
				if (iCountNum == MAX_COUNT_NUMBER) {
					/**
					 * 创建一个新的工作表 说明:如果用createSheet(String fileName)方法，当filename有中文字符时，会出现编码问题。 为解决这个问题，我们使用createSheet(),setSheetName()两个方法结合使用
					 */
					sheetTmp = workBook.createSheet();
				...
				if (sTypes == null || sTypes.length == 0) {
					...
					for (String m : aValues.values()) {
						...
						if (sTypes[j].equals(ExcelUtil.STYLE_STRING)) {// 文本类型
							...
							/**
							 * HSSFDataFormat内置的编码格式只有"m/d/yy" or "m/d/yy h:mm" 没有"mm/dd/yyyy" ,"yyyy年MM月dd日"的编码格式(有待解决)
							 */
							cell.setCellStyle(cellStyle);
							// 将字符串转化成日期类型
							SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		...
		if (response != null) {
			/*
			 * ContentType指定文件类型 可以为application/ms-excel application/ms-word application/ms-txt application/ms-html 或其他浏览器可直接支持文档
			 */
			response.reset();
			response.setContentType("application/ms-excel");
			String sSimpleName = sFileName.substring(sFileName.lastIndexOf("/") + 1, sFileName.length());
			response.setHeader("Content-Disposition", "attachment;filename=" + new String(sSimpleName.getBytes(), "ISO-8859-1"));

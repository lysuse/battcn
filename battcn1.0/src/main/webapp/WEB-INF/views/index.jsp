			...
			<div class="sidebar-collapse">
				<ul class="nav" id="side-menu">
					...
					<c:forEach items="${list}" var="rs">
						<li>
							<a href="#"> <i class="${rs.icon}"></i> <span class="nav-label">${rs.name}</span> <span class="fa arrow"></span></a>
							<ul class="nav nav-second-level">
								<c:forEach items="${rs.children}" var="ch">
									<li>
										<a title="${ch.description}" class="J_menuItem" href="${ctx}${ch.resUrl}?id=${ch.id}">${ch.name}</a>
									</li>
								</c:forEach>
							</ul>
						</li>
					...
					<!-- 该功能可选：方便管理登陆用户而已 -->
					<li><a title="Session管理" class="J_menuItem" href="${ctx}/session/list.shtml"><i class="fa fa-database"></i>Session管理</a></li>
				</ul>
			</div>
		</nav>
		<!--左侧导航结束-->
